var isDark = dbfmplayerSetting.theme === undefined ? true : dbfmplayerSetting.theme === "dark" ? true : false;
var svgFillColor = isDark ? "#4a4a4a" : "#d1d9e1";
var svgInactiveColor = "rgb(155, 155, 155)";
var volumeBarBgColor = isDark ? "#e5e5e8" : "#9b9b9b";
var volumeBarTintColor = isDark ? "#9b9b9b" : "#d1d9e1";
var cssFile = "dbfmplayer"+(isDark ? "-dark" : "-light");

// load CSS
document.getElementsByTagName("head")[0].innerHTML += '<link rel="stylesheet" type="text/css" href="http://7u2sl0.com1.z0.glb.clouddn.com/dbfmplayer/'+cssFile+'.css">'
...
musicVar += "							<path d=\"M6 4l20 12-20 12z\" fill=\""+svgFillColor+"\"><\/path>";
musicVar += "						<\/svg>";
...
document.getElementById("dbfmplayer-play").onclick = function(e) {
	if (music.paused) {
		music.play();
		document.getElementById("dbfmplayer-play").innerHTML = '<path d="M4 4h10v24h-10zM18 4h10v24h-10z" fill="'+svgFillColor+'"></path>';
	} else {
		music.pause();
		document.getElementById("dbfmplayer-play").innerHTML = '<path d="M6 4l20 12-20 12z" fill="'+svgFillColor+'"></path>';
	}
};
document.getElementById("dbfmplayer-loop").onclick = function(e) {
	music.loop = !music.loop;
	if (music.loop) {
		document.getElementById("dbfmplayer-loop").children[0].style.fill = svgFillColor;
	} else {
		document.getElementById("dbfmplayer-loop").children[0].style.fill = svgInactiveColor;
	}
};
